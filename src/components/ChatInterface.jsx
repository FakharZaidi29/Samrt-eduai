      ...
      <div className="px-3 py-2 bg-slate-50 dark:bg-zinc-800/60 border-t border-slate-100 dark:border-zinc-700 flex items-center justify-between">
        ...
        {watchUrl && (
          <a href={watchUrl} target="_blank" rel="noopener noreferrer"
            className="text-[11px] text-red-500 hover:text-red-600 flex items-center gap-1 flex-shrink-0 ml-2">
  ...
  return (
    <div className="my-3">
      <a
        ...
        className="flex items-center gap-3 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/40 rounded-xl hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors group"
      >
        <div className="w-9 h-9 rounded-lg bg-red-600 flex items-center justify-center flex-shrink-0">
          <Play size={14} className="text-white ml-0.5" fill="white" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-red-700 dark:text-red-300 truncate">{videoTitle}</p>
          <p className="text-[11px] text-red-500 dark:text-red-400 mt-0.5">Click to search on YouTube →</p>
        </div>
...
function renderContent(content) {
  const formatInline = (text) =>
    text.split(/(\*\*[^*]+\*\*|`[^`]+`)/g).map((part, j) => {
      ...
      if (part.startsWith('`') && part.endsWith('`') && part.length > 2)
        return (
          <code key={j} className="bg-slate-200/70 dark:bg-zinc-700 text-red-600 dark:text-red-300 px-1.5 py-0.5 rounded text-[11px] font-mono">
            {part.slice(1, -1)}
          </code>
        );
      return part;
  ...
  return content.split('\n').map((line, i) => {
    ...
    if (trimmed.startsWith('`') && trimmed.endsWith('`') && trimmed.length > 2) {
      return (
        <div key={i} className="my-2">
          <code className="block bg-slate-100 dark:bg-zinc-800 text-red-600 dark:text-red-300 px-4 py-2.5 rounded-xl text-xs font-mono border border-slate-200 dark:border-zinc-700">
            {trimmed.slice(1, -1)}
          </code>
        </div>
      );
    ...
    if (numMatch) {
      return (
        <div key={i} className="flex gap-2 text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
          <span className="text-red-500 font-semibold flex-shrink-0 w-5">{numMatch[1]}.</span>
...
function AIMessage({ message }) {
  ...
  return (
    <div className="flex items-start gap-3 group animate-[fadeIn_0.2s_ease-out]">
      <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-red-600 to-black flex items-center justify-center flex-shrink-0 shadow-sm shadow-red-200/50 dark:shadow-red-900/30">
        <Sparkles size={14} className="text-white" />
      </div>

      <div className="flex-1 min-w-0 max-w-2xl">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-xs font-semibold text-red-600 dark:text-red-400">EduAI Tutor</span>
        ...
        <div className="flex items-center gap-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          ...
          <button className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors">
            <RefreshCw size={11} />
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex items-start gap-3 flex-row-reverse animate-[fadeIn_0.2s_ease-out]">
      ...
      <div className="flex-1 min-w-0 flex flex-col items-end max-w-xl">
        ...
        <div className="bg-red-600 text-white rounded-2xl rounded-tr-none px-4 py-3 shadow-sm shadow-red-200/40 dark:shadow-red-900/30">
...
function TypingIndicator() {
  return (
    <div className="flex items-start gap-3">
      <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-red-600 to-black flex items-center justify-center flex-shrink-0">
        <Sparkles size={14} className="text-white" />
      </div>
...
function EmptyState({ onNewChat }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
      <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-red-600 to-black flex items-center justify-center mb-4 shadow-lg shadow-red-900/30">
      ...
      <button
        onClick={onNewChat}
        className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors shadow-sm shadow-red-200/50 dark:shadow-red-900/30"
      >
        <Plus size={16} />
        New Chat
      </button>
      ...
      <div className="hidden md:flex flex-col w-64 border-r border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex-shrink-0">
        ...
        <div className="p-4 border-b border-slate-100 dark:border-zinc-800">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 active:scale-[0.98] text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-all duration-150 shadow-sm shadow-red-200/50 dark:shadow-red-900/30"
          >
            <Plus size={16} />
            New Chat
          </button>
        ...
        <div className="px-3 py-3">
          <div className="flex items-center gap-2 bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-xl px-3 py-2 focus-within:border-red-300 dark:focus-within:border-red-700 transition-colors">
                ...
                <div key={session._id} className="relative group/item mb-0.5">
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <MessageSquare
                        size={13}
                        className={`mt-0.5 flex-shrink-0 ${isActive ? 'text-red-500' : 'text-slate-400'}`}
                      />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1">
                          <p className={`text-xs font-medium truncate ${isActive ? 'text-red-700 dark:text-red-300' : 'text-slate-700 dark:text-slate-300'}`}>
                            {session.title}
                          </p>
                  ...
                  <button
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-lg text-slate-300 dark:text-zinc-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover/item:opacity-100 transition-all duration-150"
                    title="Delete chat"
                  >
                    <Trash2 size={12} />
                  </button>
          ...
          <>
            ...
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex-shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-red-600 to-black flex items-center justify-center shadow-sm shadow-red-200/40 dark:shadow-red-900/30">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
              {messagesLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center py-16">
                  <Sparkles size={28} className="text-red-400 mb-3" />
            ...
            <div className="px-5 pb-5 flex-shrink-0">
              <div className="flex items-end gap-3">
                <div className="flex-1 bg-slate-50 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700 rounded-2xl px-4 py-3 focus-within:border-red-300 dark:focus-within:border-red-600 focus-within:ring-2 focus-within:ring-red-100/60 dark:focus-within:ring-red-900/40 transition-all duration-200">
                  <textarea
                    ref={textareaRef}
                    value={input}
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-100 dark:border-zinc-700/60">
                    <div className="flex items-center gap-0.5">
                      <button className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Attach file">
                        <Paperclip size={15} />
                      </button>
                      <button className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Voice input">
                        <Mic size={15} />
                      </button>
                    </div>
                ...
                <button
                  ...
                  className={`p-3.5 rounded-2xl transition-all duration-200 flex-shrink-0 ${
                    input.trim() && !isTyping
                      ? 'bg-red-600 hover:bg-red-700 active:scale-95 text-white shadow-sm shadow-red-200/50 dark:shadow-red-900/30'
